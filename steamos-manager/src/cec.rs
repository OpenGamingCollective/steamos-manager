...
mod test {
    ...
    // Multi-threaded so a spinning run loop can't starve the timeout below:
    // on the default current-thread runtime this test would hang instead of
    // failing.
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn test_cecd_service_bus_disconnect() {
        // Losing the bus terminates the signal streams. They then yield `None`
        // immediately and forever, so the run loop must exit rather than spin
        // on them, which would peg a core and starve the cancellation token.
        let test = setup_cec_hw_test(MockCecHw {
            can_awaken: true,
            phys_addr: 0xFFFF,
            awaken: false,
